    ...
    <body class="bg-[#F8F8FF]">
        ...
        <div class="container mx-auto">
            ...
            <div class="content relative top-[50px] lg:top-[50px] w-[80%] mx-auto">
                ...
                <!-- Decision Card -->
                @if ($trigerPenanganan != null && $userHealth != null)
                    <div class="max-w-md p-6 bg-white rounded-lg shadow-lg border border-gray-200">
                        <div class="flex items-center justify-between flex-col mb-4">
                            ...
                            <div>
                                ...
                                <span class="text-center text-xs">({{ getTrigerPenanganan($penanganan->id_penanganan, $userHealth->level)->judul }})</span>
                            </div>
                        </div>
                        <!-- Progress Circle -->
                        <div class="lg:flex-[0_0_120px] flex-[0_0_100px]">
                            <div class="ldBar label-center" data-min="0" data-max="{{ count($trigerPenanganan) }}" data-value="{{ $userHealth->level }}" data-preset="circle" style="width: 100%;height: 90px;"></div>
                            <style>
                                .ldBar-label:after {
                                    content: " / {{ count($trigerPenanganan) }}";
                                }
                            </style>
                        </div>
                        <p class="mt-4 text-center text-gray-600">
                            ...
                            @if ($userHealth->status == 'stagnasi' && count($log_treatment) >= 3)
                                ...
                                @endif
                            @else
                                <span>{{ getTrigerPenanganan($penanganan->id_penanganan, $userHealth->level)->{$userHealth->status} }}</span>
                            @endif
                        </p>
                    </div>
                @endif
                ...
                @if ($penanganan->video)
                    ...
                    <iframe class="w-[100%] h-[50vw] mt-5 mb-10" src="https://www.youtube.com/embed/{{ $penanganan->video }}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen>
                    </iframe>
                @endif
                    ...
                    {{-- <a href="{{ route('add-log-penanganan', ['id' => $penanganan->id_penanganan]) }}">
                        ...
                        <script>
                            ...
                            Swal.fire({
                                ...
                                text: '{{ strip_tags($trigerpenanganan->first()->konten) }}', // Menampilkan konten dari penanganan pertama
                                icon: 'info',
                    ...
                    <div class="content">
                        ...
                        <div class="overflow-x-auto">
                            <table class="border-2 border-black">
                                ...
                                <tbody>
                                    @foreach ($trigerPenanganan as $item)
                                        <tr>
                        ...
                        <form id="input_decision" class="mt-2 {{ count($log_treatment) != 0 ? 'hidden' : '' }}" method="POST" action="{{ route('update-perkembangan-pasien') }}">
                            ...
                            <input type="hidden" name="jenis" value="{{ base64_encode('penanganan') }}">
                            <input type="hidden" name="id" value="{{ base64_encode($penanganan->id_penanganan) }}">
                            <input type="number" id="perubahan_decision" value="{{ $userHealth ? $userHealth->level : 0 }}" maxlength="1" min="0" minlength="1" max="{{ count($trigerPenanganan) }}" name="perubahan_decision" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukan Level Baru">
                        </form>
                    </div>
            ...
            document.getElementById('button-dilakukan').addEventListener('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // buat redirect ke {{ route('add-log-penanganan', ['id' => $penanganan->id_penanganan]) }}
                        window.location.href = "{{ route('add-log-penanganan', ['id' => $penanganan->id_penanganan]) }}";
                    }
                });
            });
        </script>
