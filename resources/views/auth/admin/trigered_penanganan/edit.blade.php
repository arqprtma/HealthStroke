...
<html lang="en">
    ...
    <body class="bg-[#F8F8FF]">
        ...
        <form method="post" action="{{ route('admin.trigered.penanganan.update', $trigerPenanganan->id_trigered_penanganan) }}" class="container lg:w-[80%] mx-auto pb-7 px-5 lg:-mt-20 -mt-24">
            @csrf
            @method('PUT')
            <div class="lg:w-[80%] w-full rounded-md shadow-md bg-[#F2F2F2] p-3 lg:mx-auto">
                <div>
                        ...
                        <div class="relative">
                            <select id="penanganan" name="penanganan" class="select2 block appearance-none w-full bg-white border border-gray-300 hover:border-gray-400 text-gray-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:shadow-outline focus:border-[#15ADA7]">
                                ...
                                @foreach ($penanganan as $data)
                                    <option class="two-lines" value="{{ $data->id_penanganan }}" {{ $data->id_penanganan == $trigerPenanganan->id_penanganan ? 'selected' : '' }}>{!! $data->deskripsi !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input value="{{ $trigerPenanganan->judul }}" type="text" id="judul" name="judul" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan Judul">
                        <span class="text-xs text-red-500">* Contoh : Tidak ada kontraksi atau reaksi otot</span>
                    </div>
                    <!-- Input Number -->
                    <div class="mb-4">
                        <label for="level" class="block text-sm font-medium text-gray-700">Level</label>
                        <input value="{{ $trigerPenanganan->level }}" type="number" id="level" name="level" min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="0">
                    </div>
                </div>
                <!-- Textarea -->
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ $trigerPenanganan->kemajuan }}" type="text" id="kemajuan" name="kemajuan" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan kalimat kemajuan">
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ $trigerPenanganan->kemunduran }}" type="text" id="kemunduran" name="kemunduran" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan kalimat kemunduran">
        ...
        <script>
            ...
            $(document).ready(function(){
                let value = '{!! $trigerPenanganan->konten !!}';
