...
<body class="bg-[#F8F8FF]">
    <div class="bg-auto bg-cover bg-left-top bg-no-repeat -mt-4 lg:h-[356px] h-[300px]" id="bg-blub">
        ...
        <div class="container lg:w-[80%] mx-auto pb-7 px-5">
            <h2 class="text-center font-bold text-black lg:text-2xl text-lg mb-10">Trigger penanganan</h2>
            <a href="{{ route('admin.trigered.penanganan.tambah') }}"><button
    ...
    <div class="container lg:w-[80%] mx-auto pb-7 px-5 lg:-mt-5 mt-7">
        ...
        <table id="tables" class="w-full">
            ...
            <tbody>
                <?php $i = 1?>
                @foreach ($trigeredpenanganan as $item)
                <tr>
                    <td>{{$i}}</td>
                    ...
                    <form id="deleteForm{{ $item->id_trigered_penanganan }}" action="{{ route('admin.trigered.penanganan.delete',  ['id' => $item->id_trigered_penanganan]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
                </tr>
