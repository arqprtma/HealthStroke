    ...
    <body class="bg-[#F8F8FF]">
        <div class="flex items-center bg-auto bg-cover bg-left-top bg-no-repeat -mt-4 lg:h-[356px] h-[300px]" id="bg-blub">
            <div class="px-5 flex items-center justify-center lg:ms-[250px] gap-2">
                <a href="{{ route('admin.trigered.aktivitas') }}" class="flex-shrink-0">
                    <img src="{{ asset('images/icons/arrow-left.png') }}" width="30px" alt="Back Arrow">
                </a>
                <h2 class="text-left font-bold text-black lg:text-xl text-sm m-0">Trigger Aktivitas</h2>
            </div>
        </div>

        <form method="post" action="{{ route('admin.trigered.aktivitas.update', $trigerAktivitas->id_trigered_aktivitas) }}" class="container lg:w-[80%] mx-auto pb-7 px-5 lg:-mt-20 -mt-24">
            @csrf
            @method('PUT')
            <div class="lg:w-[80%] w-full rounded-md shadow-md bg-[#F2F2F2] p-3 lg:mx-auto">
                <div>
                    <!-- Select Option -->
                    <div class="mb-4">
                        <label for="aktivitas" class="block text-sm font-medium text-gray-700">Pilih Aktivitas</label>
                        <div class="relative">
                            <select id="aktivitas" name="aktivitas" class="select2 block appearance-none w-full bg-white border border-gray-300 hover:border-gray-400 text-gray-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:shadow-outline focus:border-[#15ADA7]">
                                <option value="">-- Pilih --</option>
                                @foreach ($aktivitas as $data)
                                    <option class="two-lines" value="{{ $data->id_aktivitas }}" {{ $data->id_aktivitas == $trigerAktivitas->id_aktivitas ? 'selected' : '' }}>{!! $data->deskripsi !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input value="{{ $trigerAktivitas->judul }}" type="text" id="judul" name="judul" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan Judul">
                        <span class="text-xs text-red-500">* Contoh : Tidak ada kontraksi atau reaksi otot</span>
                    </div>
                    <!-- Input Number -->
                    <div class="mb-4">
                        <label for="level" class="block text-sm font-medium text-gray-700">Level</label>
                        <input value="{{ $trigerAktivitas->level }}" type="number" id="level" name="level" min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="0">
                    </div>
                </div>
                <!-- Textarea -->
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ $trigerAktivitas->kemajuan }}" type="text" id="kemajuan" name="kemajuan" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan kalimat kemajuan">
                ...
                <div class="mb-4">
                    ...
                    <input value="{{ $trigerAktivitas->kemunduran }}" type="text" id="kemunduran" name="kemunduran" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-[#15ADA7] focus:border-[#15ADA7]" placeholder="Masukkan kalimat kemunduran">
        ...
        <script>
            ...
            // Set Nilai pada CKEditor
            $(document).ready(function(){
                let value = '{!! $trigerAktivitas->konten !!}';
                
                setEditorContent('konten', value);
            });
        </script>
