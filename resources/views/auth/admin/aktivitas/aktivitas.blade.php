...
<body class="bg-[#F8F8FF]">
    <div class="flex items-center justify-around bg-auto bg-cover bg-left-top bg-no-repeat -mt-4 lg:h-[356px] h-[300px]" id="bg-blub">
        <div class="px-5">
            <a href="{{ route('admin.aktivitas') }}" class="flex gap-2 mb-2">
                <img src="{{ asset('images/icons/arrow-left.png ') }}" width="30px" alt="Back Arrow">
            </a>
            <h2 class="text-left font-bold text-black lg:text-xl text-sm m-0">Aktivitas Treatment</h2>
        </div>
        <div class="px-5">
            <a href="{{ route('admin.aktivitas.kategori.tambah') }}"><button class="px-2 py-2 bg-[#8DD67A] hover:bg-[#85D470] text-white rounded ml-auto block lg:text-base text-sm" id="dropdown">Tambah Kategori
                Aktivitas</button></a>
        </div>
    </div>
    <form method="post" action="{{ route('admin.aktivitas.store') }}" class="container lg:w-[80%] mx-auto pb-7 px-5 lg:-mt-20 -mt-24">
        @csrf
