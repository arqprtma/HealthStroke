...
<body class="bg-[#F8F8FF]">
    ...
    <form method="post" action="{{ route('admin.trigered.aktivitas.update', $trigeredAktivitas->id_trigered_aktivitas) }}"
        ...
        <div class="lg:w-[80%] w-full rounded-md shadow-md bg-[#F2F2F2] p-3 lg:mx-auto">
            <div>
                <label for="penanganan" class="text-base">Aktivitas</label>
                <div class="relative inline-block w-full mt-1 mb-2">
                    <select name="id_aktivitas" id="id_aktivitas"
                    ...
                    <option value=" {{$trigeredAktivitas->id_aktivitas}}"  selected>{{strip_tags($trigeredAktivitas->deskripsi)}}</option>
                    @foreach ($aktivitas as $data)
                    <option value="{{ strip_tags($data->id_aktivitas) }}">{{ \Illuminate\Support\Str::words(strip_tags($data->deskripsi), 6, '...') }}</option>
                    @endforeach
                </select>
                    <div
            ...
            <div>
                ...
                <input type="text"
                    class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline lg:text-base text-sm mt-2"
                    name="jumlah" id="jumlah" value=" {{$trigeredAktivitas->jumlah}}">
