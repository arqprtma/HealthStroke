...
<body class="bg-[#F8F8FF]">
    <div class="bg-auto bg-cover bg-left-top bg-no-repeat -mt-4 lg:h-[356px] h-[300px]" id="bg-blub">
        ...
        <div class="container lg:w-[80%] mx-auto pb-7 px-5">
            ...
            <button class="px-5 py-2 bg-[#8DD67A] hover:bg-[#85D470] text-white rounded ml-auto block" id="dropdown" data-target="#pemicu">Tambah</button>
            <form method="POST" action="{{ route('admin.pemicu.post') }}" class="flex align-items-center flex-nowrap flex-row mt-3 hidden" id="pemicu">
                @csrf
                <input type="text" name="pemicu" id="pemicu" placeholder="Tambah Pemicu" class="border border-black rounded lg:w-80 md:w-80 w-full p-2 mr-2 ml-auto">
                ...
                @foreach ($pemicu as $key => $data)
                    <tr class="{{ ($key % 2 == 1) ? 'bg-[#8DD67A] bg-opacity-30' : '' }}">
                        ...
                        <td class="border w-full min-w-[200px] p-3">
                            <form action="{{ route('admin.pemicu.update', ['id' => $data->id_pemicu]) }}" method="post" id="updateForm{{ $data->id_pemicu }}">
                                @csrf
                                @method('PUT')
                        ...
                        <td class="border w-full min-w-[130px] p-3 text-center">
                            <a href="javascript:void(0)" onclick="ConfirmUpdate('{{ $data->id_pemicu }}')" class="text-blue-500 inline-block">Edit</a> | <a href="javascript:void(0)" onclick="ConfirmDelete('{{ $data->id_pemicu }}')" class="text-red-500 inline-block">Hapus</a>
                        </td>
                    </tr>
                    <!-- Form untuk metode DELETE -->
                    <form class="hidden" id="deleteForm{{ $data->id_pemicu }}" action="{{ route('admin.pemicu.delete', ['id' => $data->id_pemicu]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                    </form>
                @endforeach
