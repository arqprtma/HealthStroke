...
<html lang="en">
    ...
    <body class="bg-[#F8F8FF]">
        ...
        <form method="post" action="{{ route('admin.trigered.aktivitas.store') }}" class="container lg:w-[80%] mx-auto pb-7 px-5 lg:-mt-20 -mt-24">
            @csrf
                        ...
                        <div class="relative">
                            <select id="aktivitas" name="aktivitas" class="select2 block appearance-none w-full bg-white border border-gray-300 hover:border-gray-400 text-gray-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:shadow-outline focus:border-[#15ADA7]">
                                ...
                                @foreach ($aktivitas as $data)
                                    <option class="two-lines" value="{{ $data->id_aktivitas }}">{!! $data->deskripsi !!}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
